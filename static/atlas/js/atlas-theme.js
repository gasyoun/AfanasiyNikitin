...
(function(){
  ...
  // H486 §2 defect 4: an already-rendered same-origin iframe reads localStorage
  // only on load — it never sees a live toggle in the parent (or a sibling
  // frame). The 'storage' event fires in every OTHER same-origin browsing
  // context when localStorage changes, so this closes that gap for both the
  // standalone pages and pages embedded via <AtlasFigure>.
  window.addEventListener('storage', e => {
    if (e.key === 'theme' && e.newValue) {
      html.setAttribute('data-theme', e.newValue);
      if (icon) {
        icon.className = e.newValue === 'dark' ? 'ti ti-sun' : 'ti ti-moon';
      }
    } else if (e.key === 'lang' && e.newValue) {
      updateLang(e.newValue);
    }
  });
