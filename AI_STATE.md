...
**Next:** (1) merge PRs #4 + #5; (2) `calendar` is **Phase 3** (generated by a reproducible computus script, not hand-entered); (3) page-level `source_page` provenance where blank (manual — needs the book); (4) **Phase 2 — Linked Open Data**: OpenRefine reconciliation of the 26 places + 15 people to Wikidata/Pleiades/VIAF, then emit Linked Places Format GeoJSON for the World Historical Gazetteer.
