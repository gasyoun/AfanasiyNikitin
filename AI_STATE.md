...
**Next:** (1) merge PRs #4 + #5; (2) `fragments` dataset (Ф.1–104 from `afanasy_manuscript_layers.html`; **metadata only** — quote field gated by RIGHTS.md) — the last data-only Phase 1 dataset; (3) `calendar` is **Phase 3** (generated by a reproducible computus script, not hand-entered); (4) page-level `source_page` provenance (manual — needs the book); (5) Phase 2 — OpenRefine reconciliation to Wikidata/Pleiades + emit Linked Places Format GeoJSON for the World Historical Gazetteer.
