...
**Next:** (1) merge the two PRs; (2) page-level `source_page` provenance (manual — needs the book); (3) remaining datasets — calendar (Phase 3, generated by a computus script), fragments (Ф.1–104, quote field gated by RIGHTS.md), trade (flagged `model`), editions, citations; (4) Phase 2 — OpenRefine reconciliation to Wikidata/Pleiades + emit Linked Places Format GeoJSON for the World Historical Gazetteer.
